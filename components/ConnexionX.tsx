...
interface ProprietesConnexionX {
...
/**
 * Composant de connexion/liaison de compte pour la plateforme Twitter/X.
 */
export default function ConnexionX({ 
  idUtilisateur, 
  estConnecte, 
  nomUtilisateur, 
  estEnCoursDeChargement 
}: ProprietesConnexionX) {
  
  /**
   * Redirige l'utilisateur vers la route d'authentification X (OAuth2).
   */
    ...
    return (
      <div className="flex items-center gap-3 w-full">
        <div className="w-9 h-9 rounded-full bg-sky-950/40 border border-sky-900/60 flex items-center justify-center shrink-0">
          <span className="text-[11px] font-bold text-sky-400">@{nomUtilisateur[0]?.toUpperCase()}</span>
        </div>
  ...
  return (
    <div className="flex flex-col gap-2.5 w-full">
      <div className="flex items-center gap-2.5 text-zinc-500">
        ...
        <div>
          <h4 className="text-xs font-medium text-zinc-400">Statut Twitter</h4>
